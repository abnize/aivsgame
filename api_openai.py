...
# ------------------------------------------------------------
def _topic(level):
    if level <= 2: return "초등수준 계산·기초 상식"
    elif level == 3: return "유명 한국어 넌센스 퀴즈(인터넷에서 많이 알려진 것만)"
    elif level == 4: return "한국 기초상식·생활 상식"
    elif level == 5: return "중학교 수준 역사·과학·지리"
    elif level == 6: return "속담·문화·음식 상식"
    elif level == 7: return "고등학교 수학·물리·국사"
    elif level == 8: return "심화 상식·추리"
    elif level == 9: return "한국 유머·기발한 문제"
    else: return "대학 교양·전문적 사고 문제"

# ------------------------------------------------------------
# 🔰 유명 넌센스 문제 (레벨 3에서만 사용)
# ------------------------------------------------------------
NONSENSE_BANK = [
...
🎯 난이도 규칙(매우 중요):
- 생성되는 퀴즈는 반드시 현재 레벨의 난이도 범위 안에서만 만들어야 한다.
- 난이도를 절대 벗어나지 마라.
...
🎯 사담 기반(memory:true) 규칙:
{"- memory:true 문제는 반드시 1개 포함해야 함." if must_memory else "- memory:true 문제는 절대 생성하지 마라."}
